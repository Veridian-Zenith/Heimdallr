...
|---|---|---|---|
| Core | C# .NET, TechnitiumLibrary.Net wire | Rust hickory-proto+hickory-server (ring) | M1–M3 |
| Runtime | GC + libmsquic for DoQ/H3 | no GC, quinn+rustls ring | no native msquic |
| Crypto | TechnitiumLibrary.Security.Cryptography | ring default, botan opt-in | no OpenSSL/BoringSSL default |
...
| Console | DnsServerCore/www + APIDOCS.md | axum :5380 API M6–M7 | JSON shapes match |
| Auth | RBAC + API tokens + TOTP + OIDC | api/auth argon2id+TOTP+OIDC | M7 |
| Clustering | manage N instances | cluster/ M8 | 2-node gate |
| Observability | Stats + query logs + LogExporter | query.log JSON + sqlite, Prometheus | M6 |
| Apps | 27 C# per-app csproj | apps/ WASM sandboxed DnsApp trait | apps2.json compat |
