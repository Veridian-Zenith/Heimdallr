...
- [M6 Filtering, Apps & Observability](#m6-filtering-apps--observability)
- [M7 Administration & Hardening](#m7-administration--hardening)
- [M8 Auxiliary Services](#m8-auxiliary-services)
- [M9 Full Parity & Migration](#m9-full-parity--migration)
- [Non-goals](#non-goals)

## M0 Scaffold ✅

**Gate:** `cargo check` and `cargo build --release` produce `target/release/heimdallr` with `--help` showing `listen`/`api_listen`, `LICENSE` is `OSL-3.0`, docs tree exists.
...
**Scope:** Validation for recursive/forwarder with `RSA`/`ECDSA`/`EdDSA`, `NSEC`/`NSEC3`, trust anchor management, DANE `TLSA` `RFC 6698`, `ZONEMD` `RFC 8976` for secondary. Signing for hosted zones (`RRSIG`/`DNSKEY`/`DS`). Crypto: `ring` by default, `Botan` optional via `--features botan-crypto` for HSM/agile.
...
**Scope:** Self-host `DoT` `RFC 7858` (`rustls` `ring`), `DoH` `RFC 8484` with `HTTP/1.1`+`HTTP/2` (`axum`+`h2`), `DoQ` `RFC 9250` (`quinn` `ring`), `DoH/3` later via `quinn`+`h3`, `PROXY protocol v1/v2` for `UDP`+`TCP`, forwarder routing over `DoT`/`DoH`/`DoQ`.
...
**Scope:** `SVCB`/`HTTPS` `RFC 9460`, `URI` `RFC 7553`, `SSHFP` `RFC 4255`, `DNAME` `RFC 6672`, proprietary `ANAME` (apex CNAME flattening) + `APP` record dispatch, QNAME minimization `RFC 9156`, QNAME case randomization `draft-vixie-dnsext-dns0x20-00`, CNAME cloaking block, `DANE` hash auto-gen from `PEM`, `DNS64` `RFC 6147`, `EDNS Client Subnet` `RFC 7871`, `EDNS EXPIRE` `RFC 7314`.
...
**Scope:** One-or-more blocklist URLs, `regex` blocklists per-client/subnet, `DnsBlockListApp` domain+IP lists, `BlockPageApp` HTTP sinkhole, DNS Rebinding Protection, `DropRequestsApp`/`NxDomainApp`, latency-based forwarder selection (concurrency), persistent cache save/restore, stats + query logs, Prometheus metrics, HTTP API (auth, zones, settings, DHCP, logs).

**Gate:** Blocklist blocks `ads.example.test.`, per-client override allows it for `10.0.0.5/32`; API `GET /api/listBlocked` etc; logs export via `Syslog`/`HTTP`/`File`.

## M7 Administration & Hardening

**Scope:** Web console (`axum` + static files, dark mode), multi-user RBAC + non-expiring API tokens, `TOTP` 2FA, `Single Sign-On` `OIDC`, system logging, `EDNS`/ECS toggles, split-horizon/geo via Apps, clustering stub.
...
**Scope:** Built-in DHCP Server (multi-scope), HTTP/SOCKS5 proxy routing (incl. Tor), Cluster management of N instances, Docker `Dockerfile`+`docker-compose.yml`, `systemd.service` install.
...
**Scope:** Remaining RFC gaps, `Secondary` `XFR-over-TLS` `RFC 9103` + `XFR-over-QUIC` `RFC 9250`, `TSIG` `RFC 8945` for transfers/updates `RFC 2136`, `FilterAaaaApp`/`WeightedRoundRobinApp`/`FailoverApp` with health checks, import Technitium zone JSON, bulk `AdvancedForwardingApp` compat, benchmark target: >60k qps on `i7-8700` class (`Gigabit` `UDP`).

**Gate:** Import Technitium backup zones, all queries match before/after `diff` of `dig` traces; `perf` bench shows `p95 <8ms` recursive cold, `>60k qps` cached.

## Non-goals

Windows service, legacy PostgreSQL query logs — fresh `sqlite`/`postgres` plugins instead.
